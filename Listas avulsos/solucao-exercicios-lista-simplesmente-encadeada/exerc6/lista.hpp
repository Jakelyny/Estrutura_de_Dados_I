...
void naoSeguras(No **senhas, No **senhasNS)
{
    ...
    while (n)
    {
        if(n->dado.size()< 4 || contaNoL(senhas, n->dado) > 5) //verifica se a senha é "não segura"
        {
            if(contaNoL(senhasNS, n->dado) == 0) //para não inserir senhas repetidas na lista
...
bool removeInicioL(No **lista, DadoNoLista *valor)
{
    ...
    //pega a referencia do primeiro nó e seu valor
    No *no = *lista;
    *valor = no->dado;

    //atualiza o inicio da lista para o próximo nó
    *lista = no->prox;
