...
bool insereFimL(Lista *lista, DadoNoLista valor)
{
    No *novo = new No();
    ...
    novo->dado = valor;
    novo->prox = nullptr; //O ponteiro *prox do ultimo nó sempre aponta pra NULL
...
bool removeFimL(Lista *lista)
{
    ...
    No *anterior = lista->fim->ant;

    lista->tamanho--;
    delete lista->fim;

    if(!anterior)  //A lista ficou vazia ???
    {
        lista->fim = nullptr;
